...
for ix=x_cell:x_cell+3
    for iy=y_cell:y_cell+3
        for iz=z_cell:z_cell+3   
            ...
            AAr=AAr+d(ix,iy,iz)*f_x(ix-x_cell+1,1)*f_y(iy-y_cell+1,1)*f_z(iz-z_cell+1,1);
             
            AAtheta=AAtheta+c(ix,iy,iz)*f_x(ix-x_cell+1,1)*f_y(iy-y_cell+1,1)*f_z(iz-z_cell+1,1);
