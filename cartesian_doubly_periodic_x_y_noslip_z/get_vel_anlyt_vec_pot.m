...
global hx hy hz c d nx ny nz xmin ymin zmin;
...
for iz=z_cell:z_cell+3  
    for iy=y_cell:y_cell+3
        for ix=x_cell:x_cell+3
            ...
            dAydz=dAydz+d(ix,iy,iz)*f_x(ix-x_cell+1,1)*f_y(iy-y_cell+1,1)*f_z(iz-z_cell+1,2);
            dAxdz=dAxdz+c(ix,iy,iz)*f_x(ix-x_cell+1,1)*f_y(iy-y_cell+1,1)*f_z(iz-z_cell+1,2);
            dAydx=dAydx+d(ix,iy,iz)*f_x(ix-x_cell+1,2)*f_y(iy-y_cell+1,1)*f_z(iz-z_cell+1,1);
            dAxdy=dAxdy+c(ix,iy,iz)*f_x(ix-x_cell+1,1)*f_y(iy-y_cell+1,2)*f_z(iz-z_cell+1,1);
